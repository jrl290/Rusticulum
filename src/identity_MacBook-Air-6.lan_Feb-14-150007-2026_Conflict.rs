...
pub struct Identity {
    ...
    encryption_prv_key: X25519PrivateKey,
    encryption_pub_key: X25519PublicKey,
    
    // Ed25519 keys for signing
    signing_prv_key: Ed25519PrivateKey,
    signing_pub_key: Ed25519PublicKey,
...
impl Clone for Identity {
    fn clone(&self) -> Self {
        let mut cloned = Identity::from_bytes(&self.get_private_key()).unwrap_or_else(|_| Identity::new(true));
...
impl Identity {
    ...
    pub fn new(_create_keys: bool) -> Self {
        // Use manual elliptic secret generation instead of keypair generator due to dependency version constraints
        ...
        let sign_keypair = ed25519_dalek::Keypair::from_bytes(&sign_key_bytes)
            .unwrap_or_else(|_| {
                // Fallback: generate new keypair
                let bytes = [0u8; 32];
                ed25519_dalek::Keypair::from_bytes(&bytes)
                    .expect("Failed to create keypair from zero bytes")
            });
        
        let mut identity = Identity {
            encryption_prv_key: enc_prv,
            encryption_pub_key: X25519PublicKey::from(&X25519PrivateKey::from(enc_key_bytes)),
            signing_prv_key: sign_keypair.secret,
            signing_pub_key: sign_keypair.public,
            hash: Vec::new(),
    ...
    pub fn create_keys(&mut self) {
        let mut enc_key_bytes = [0u8; 32];
        OsRng.fill_bytes(&mut enc_key_bytes);
        self.encryption_prv_key = X25519PrivateKey::from(enc_key_bytes);
        self.encryption_pub_key = X25519PublicKey::from(&self.encryption_prv_key);
        
        let mut sign_key_bytes = [0u8; 32];
        OsRng.fill_bytes(&mut sign_key_bytes);
        let keypair = ed25519_dalek::Keypair::from_bytes(&sign_key_bytes)
            .unwrap_or_else(|_| {
                let zero_bytes = [0u8; 32];
                ed25519_dalek::Keypair::from_bytes(&zero_bytes)
                    .expect("Failed to create keypair")
            });
        self.signing_prv_key = keypair.secret;
        self.signing_pub_key = keypair.public;
    ...
    pub fn from_bytes(prv_bytes: &[u8]) -> Result<Self, String> {
        ...
        let enc_key_bytes: [u8; 32] = prv_bytes[..32].try_into()
            .map_err(|_| "Invalid encryption key".to_string())?;
        identity.encryption_prv_key = X25519PrivateKey::from(enc_key_bytes);
        identity.encryption_pub_key = X25519PublicKey::from(&identity.encryption_prv_key);
        
        // Next 32 bytes: Ed25519 private key
        let sign_key_bytes: [u8; 32] = prv_bytes[32..64].try_into()
            .map_err(|_| "Invalid signing key".to_string())?;
        let keypair = ed25519_dalek::Keypair::from_bytes(&sign_key_bytes)
            .map_err(|e| format!("Invalid Ed25519 key: {}", e))?;
        identity.signing_prv_key = keypair.secret;
        identity.signing_pub_key = keypair.public;
    ...
    pub fn from_public_key(pub_bytes: &[u8]) -> Result<Self, String> {
        ...
        identity.encryption_pub_key = X25519PublicKey::from(enc_pub_bytes);
        identity.encryption_prv_key = X25519PrivateKey::from([0u8; 32]);

        let sign_pub_bytes: [u8; 32] = pub_bytes[32..64]
            .try_into()
            .map_err(|_| "Invalid signing public key".to_string())?;
        identity.signing_pub_key = Ed25519PublicKey::from_bytes(&sign_pub_bytes)
            .map_err(|e| format!("Invalid Ed25519 public key: {}", e))?;
        identity.signing_prv_key = Ed25519PrivateKey::from_bytes(&[0u8; 32])
            .map_err(|e| format!("Invalid Ed25519 private key seed: {}", e))?;
    ...
    pub fn get_private_key(&self) -> Vec<u8> {
        ...
        key.extend_from_slice(&self.encryption_prv_key.to_bytes());
        key.extend_from_slice(self.signing_prv_key.as_bytes());
        key
    }
    ...
    pub fn get_public_key(&self) -> Vec<u8> {
        let mut key = Vec::with_capacity(64);
        key.extend_from_slice(self.encryption_pub_key.as_bytes());
        key.extend_from_slice(self.signing_pub_key.as_bytes());
        key
    }
    ...
    fn update_hashes(&mut self) {
        ...
        self.hash = truncated_hash(&pub_key);
    }
    ...
    pub fn encrypt(&self, plaintext: &[u8]) -> Result<Vec<u8>, String> {
        ...
        // Perform ECDH
        let shared_secret = ephemeral_prv.diffie_hellman(&self.encryption_pub_key);
    ...
    pub fn decrypt(&mut self, ciphertext: &[u8]) -> Result<Vec<u8>, String> {
        ...
        self.decrypt_with_key(&self.encryption_prv_key, &ephemeral_pub, token_data)
    }
    ...
    pub fn sign(&self, data: &[u8]) -> Vec<u8> {
        ...
        let kp_secret = ed25519_dalek::Keypair::from_bytes(&self.signing_prv_key.to_bytes())
    ...
    pub fn validate(&self, signature: &[u8], data: &[u8]) -> bool {
        ...
        match Signature::from_bytes(&sig_bytes) {
            Ok(sig) => self.signing_pub_key.verify(data, &sig).is_ok(),
            Err(_) => false,
        }
    }
    ...
    pub fn remember_ratchet_local(&mut self, ratchet_id: Vec<u8>, ratchet_pub: Vec<u8>) -> Result<(), String> {
        ...
        if let Some(ref storage_path) = self.storage_path {
            let ratchet_dir = storage_path.join("ratchets").join(hex::encode(&self.hash));
    ...
    pub fn validate_announce(
        packet: &[u8],
        _destination_hash: Option<&[u8]>,
        public_key: Option<&[u8]>,
    ) -> bool {
        // This is simplified; real implementation would parse packet structure
        // and verify signature matches destination hash
        ...
        let signature = &packet[packet.len() - 64..];
        let signed_data = &packet[..packet.len() - 64];

        // Verify using provided public key
        if let Some(pub_key_bytes) = public_key {
