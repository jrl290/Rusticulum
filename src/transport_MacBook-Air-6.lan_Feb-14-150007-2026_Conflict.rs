...
impl InterfaceStub {
    ...
    pub const MODE_ACCESS_POINT: u8 = 0x01;
...
impl Transport {
    ...
    pub fn rpc_key() -> Option<Vec<u8>> {
        ...
        state
            .identity
            .as_ref()
            .map(|identity| Identity::full_hash(&identity.get_private_key()))
    }
    ...
    pub fn identity_hash() -> Option<Vec<u8>> {
        let state = TRANSPORT.lock().unwrap();
        state.identity.as_ref().map(|id| id.hash.clone())
    }
    ...
    pub fn discovery_identity_clone() -> Option<Identity> {
        ...
        Identity::from_bytes(&source.get_private_key()).ok()
    }
    ...
    pub fn path_request_handler(data: &[u8], packet: &Packet) {
        ...
        if data.len() > (crate::reticulum::TRUNCATED_HASHLENGTH / 8) * 2 {
            ...
            if !raw_tags.is_empty() && raw_tags.len() <= crate::reticulum::TRUNCATED_HASHLENGTH / 8 {
                tag_bytes = Some(raw_tags);
            }
        ...
        if let Some(tag_bytes) = tag_bytes {
            let unique_tag = [destination_hash, tag_bytes].concat();
    ...
    pub fn path_request(
        ...
        let interface_str = attached_interface.as_ref()
            .map(|i| format!(" on {}", i))
            .unwrap_or_default();
        
        log(&format!("Path request for {}{}", crate::hexrep(&destination_hash, true), interface_str), LOG_DEBUG, false, false);
        
        let state = TRANSPORT.lock().unwrap();
        
        // Check if we have a local destination for this hash
        let local_dest_index = state.destinations.iter()
            .position(|dest| &dest.hash == &destination_hash);
        
        if let Some(idx) = local_dest_index {
            drop(state);
            log(&format!("Answering path request for {}{}, destination is local to this system", 
                crate::hexrep(&destination_hash, true), interface_str), LOG_DEBUG, false, false);
            
            // Announce the local destination as a path response
            let mut state = TRANSPORT.lock().unwrap();
            if let Some(dest) = state.destinations.get_mut(idx) {
                match dest.announce(None, true, attached_interface, tag, true) {
                    Ok(_) => {
                        log(&format!("Sent path response announce for local destination"), LOG_DEBUG, false, false);
                    }
                    Err(e) => {
                        log(&format!("Error sending path response announce: {}", e), LOG_ERROR, false, false);
                    }
                }
            }
            return;
        ...
        // Check if we have a path in our path table
        if let Some(path_entry) = state.path_table.get(&destination_hash).cloned() {
            let packet_hash = match path_entry.get(IDX_PT_PACKET) {
                Some(PathEntryValue::PacketHash(hash)) => hash.clone(),
                _ => {
                    drop(state);
                    log(&format!("Could not retrieve packet hash from path table"), LOG_ERROR, false, false);
                    return;
                }
            };
            ...
            // Check if requestor is next hop - if so, don't answer
            if let Some(req_id) = &requestor_transport_id {
                if req_id == &next_hop {
                    drop(state);
                    log(&format!("Not answering path request for {}{}, since next hop is the requestor", 
                        crate::hexrep(&destination_hash, true), interface_str), LOG_DEBUG, false, false);
                    return;
                }
            }
            
            // Check for roaming-mode interface conflicts
            if let Some(req_intf) = &attached_interface {
                if let Some(recv_intf) = &received_from_intf {
                    if state.interfaces.iter().any(|i| &i.name == req_intf && i.mode == InterfaceStub::MODE_ROAMING) {
                        if req_intf == recv_intf {
                            drop(state);
                            log(&format!("Not answering path request on roaming-mode interface, since next hop is on same roaming-mode interface"), LOG_DEBUG, false, false);
                            return;
                        }
                    }
                }
            ...
            // Drop lock before potentially long-running get_cached_packet
            drop(state);
            
            // Retrieve cached announce packet
            let mut packet = match Transport::get_cached_packet(&packet_hash, Some("announce".to_string())) {
                Some(pkt) => pkt,
                None => {
                    log(&format!("Could not retrieve announce packet from cache while answering path request for {}", 
                        crate::hexrep(&destination_hash, true)), LOG_ERROR, false, false);
                    return;
                }
            };
            
            log(&format!("Answering path request for {}{}, path is known", 
                crate::hexrep(&destination_hash, true), interface_str), LOG_DEBUG, false, false);
            
            // Set the hop count from our path table
            packet.hops = announce_hops;
            
            // Prepare announce table entry
            let now = now();
            ...
            let retransmit_timeout = if is_from_local_client {
                // Local client requests are answered immediately
                now
            } else {
                // Re-acquire lock to check if next hop is local client
                let state = TRANSPORT.lock().unwrap();
                let is_next_hop_local_client = if let Some(next_hop_intf) = Transport::next_hop_interface_locked(&state, &destination_hash) {
                ...
                if is_next_hop_local_client {
                    log(&format!("Path request destination {} is on a local client interface, rebroadcasting immediately", 
                        crate::hexrep(&destination_hash, true)), crate::LOG_EXTREME, false, false);
                    now
                } else {
                    let mut timeout = now + PATH_REQUEST_GRACE;
                    // If answering on roaming-mode interface, wait longer
                    if let Some(req_intf) = &attached_interface {
                        let state = TRANSPORT.lock().unwrap();
                        if let Some(intf) = state.interfaces.iter().find(|i| &i.name == req_intf) {
            ...
            // Handle edge case: held announces
            let mut state = TRANSPORT.lock().unwrap();
            if let Some(ref dest_hash) = packet.destination_hash {
                if state.announce_table.contains_key(dest_hash) {
                    if let Some(held_entry) = state.announce_table.remove(dest_hash) {
                        state.held_announces.insert(dest_hash.clone(), held_entry);
                    }
                }
            }
            
            // Insert into announce table for scheduled rebroadcast
            ...
            drop(state);
            log(&format!("Ignoring path request for {}{}, no path known", 
                crate::hexrep(&destination_hash, true), interface_str), LOG_DEBUG, false, false);
        }
    }
    ...
    pub fn jobs() {
        ...
        if now() > state.announces_last_checked + state.announces_check_interval {
            ...
            let identity_hash = state.identity.as_ref().map(|i| i.hash.clone());
    ...
    pub fn inbound(raw: Vec<u8>, receiving_interface: Option<String>) -> bool {
        ...
        if packet.packet_type == ANNOUNCE {
            ...
            if packet.data.len() >= (crate::reticulum::TRUNCATED_HASHLENGTH / 8 + crate::identity::KEYSIZE / 8) {
                let pub_key = packet.data[(crate::reticulum::TRUNCATED_HASHLENGTH / 8)..(crate::reticulum::TRUNCATED_HASHLENGTH / 8 + crate::identity::KEYSIZE / 8)].to_vec();
                if !Identity::validate_announce(&packet.data, packet.destination_hash.as_ref().map(|v| v.as_slice()), Some(&pub_key)) {
                    should_add = false;
                }
            }
        ...
        if packet.packet_type != ANNOUNCE && packet.transport_id.is_some() {
            if let Some(identity) = &state.identity {
                if packet.transport_id.as_ref() == Some(&identity.hash) {
                    if let Some(destination_hash) = &packet.destination_hash {
    ...
    pub fn request_path(
        ...
        let path_request_data = if let Some(identity) = &TRANSPORT.lock().unwrap().identity {
            ...
            data.extend_from_slice(&identity.hash);
    ...
    pub fn packet_filter(packet: &Packet) -> bool {
        ...
        if packet.packet_type == ANNOUNCE {
            if packet.destination_type == Some(crate::destination::DestinationType::Single) {
                return true;
            }
            return false;
        }
